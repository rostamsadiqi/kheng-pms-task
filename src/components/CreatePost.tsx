...
const CreatePost: React.FC<CreatePostProps> = ({ isOpen, onClose, onCreate }) => {
	const {
		register,
		handleSubmit,
		reset,
	...
	const onSubmit = async (data: PostFormData) => {
		try {
			const response = await customAxios.post(`/api/posts/create`, data);
	...
	return (
		<div className="absolute w-full h-screen top-0 left-0 flex items-center justify-center bg-gray-500 bg-opacity-50">
			<div className="bg-[#FFFFFF] w-[280px] max-w-[280px] p-6 rounded-xl">
				<h2 className="text-lg font-semibold mb-5 text-center">Add A Post</h2>
				<form onSubmit={handleSubmit(onSubmit)}>
					...
					<div className="flex justify-around mt-4">
						...
						<button className="bg-[#F8B959] postButton" type="submit">
							Add
						</button>
					</div>
				</form>
